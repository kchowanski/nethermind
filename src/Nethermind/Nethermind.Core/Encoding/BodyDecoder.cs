...
{
    public class BodyDecoder : IRlpDecoder<BlockBody>
    {
        private HeaderDecoder _headerDecoder = new HeaderDecoder();
        private TransactionDecoder _txDecoder = new TransactionDecoder();
        
        public BlockBody Decode(Rlp.DecoderContext context, RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            ...
            // TODO: remove to array calls
            return new BlockBody(transactions.ToArray(), ommerHeaders.ToArray());
        }
        ...
        private (int Total, int Txs, int Ommers) GetContentLength(BlockBody item, RlpBehaviors rlpBehaviors)
        {   
            int contentLength = 0;
        ...
        private int GetOmmersLength(BlockBody item, RlpBehaviors rlpBehaviors)
        {
            int ommersLength = 0;
        ...
        public void Encode(MemoryStream stream, BlockBody item, RlpBehaviors rlpBehaviors = RlpBehaviors.None)
        {
            if (item == null)
            {
